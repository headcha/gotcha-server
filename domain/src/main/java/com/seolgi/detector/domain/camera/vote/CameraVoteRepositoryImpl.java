...
public class CameraVoteRepositoryImpl extends BaseQueryDslSupport<CameraVote> implements CameraVoteCustomRepository {
    ...
    @Override
    public Optional<CameraVote> findOne(long cameraId, long memberId, Candidate candidate) {
        ...
        query.where(camera.id.eq(cameraId) , cameraVote.candidate.eq(candidate) , member.id.eq(memberId) , cameraVote.disabled.isFalse());
